...
from DbServer.DbUserMsgServer import DbUserMsgServer
...
class DbMainServer:
    def __init__(self):
        ...
        self.Dgs = DbGhServer()
        self.DbUserMsg = DbUserMsgServer()
    ...
    def addUserMsg(self, wxId, roomId, question, answer):
        """
        签到
        :param wxId:
        :param roomId:
        ...
        try:
            # self.initUser(wxId, roomId)
            self.DbUserMsg.addMessage(wxId, roomId, question, answer)
            return True
        except Exception as e:
            op(f'[-]: 写入用户消息失败, 错误信息: {e}')
            return False

    def getUserMsg(self, roomId, wxId):
        """
        搜索黑名单群聊
        :return:
        """
        try:
            return self.DbUserMsg.getUserMsg(wxId, roomId)
        except Exception as e:
            op(f'[-]: 查询用户消息失败, 错误信息: {e}')
            return False
...
if __name__ == '__main__':
    Ds = DbMainServer()
    # print(Ds.clearSign())
    userList = Ds.getUserMsg('43378924022@chatroom', 'wxid_lchrch47cr1m22')
    print(userList)
