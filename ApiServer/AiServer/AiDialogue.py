...
from utils.UserContext import get_context_data
...
class AiDialogue:
    ...
    os.environ["QIANFAN_SK"] = "ms4CrIak1UfCp1RKCb5nmGjNokrvNgqK"
    os.environ["QIANFAN_AK"] = "radUp395J5CJRq06leUK04pb"
    ...
    def getQianFanAi(self, content):
        ...
        :return:
        """
        op(f'[*]: 正在调用千帆大模型对话接口... ... 请求参数' + content)
        ...
        def getAiContent(content):
            try:
                # wxId = 'wxid_lchrch47cr1m22'
                # roomId = '43378924022@chatroom'

                wxId = get_context_data()['sendWxId']
                roomId = get_context_data()['roomId'];
                userMsgList = self.Dms.getUserMsg(roomId, wxId);
                ...
                if userMsgList:
                    for user in userMsgList:
                        # 解包元组到变量中
                        wxId, roomId, question, answer, createTime = user
                        ...
                        if total_length >= 20000:
                            break  # 跳出循环
                            # 打印总长度
                op(f'[*]: 调用千帆大模型对话接口... ...message长度' + str(total_length))
                messages.append({"role": "user", "content": content})
                ...
                resp = chat_comp.do(
                    ...
                    temperature=0.9,
                    penalty_score=1.0,
                    # keyword='{"system":"你是一个娱乐圈的璀璨新星"王一博"，以其多才多艺和独特魅力征服了无数观众的心，你不仅在演艺方面展现出惊人的天赋，更在音乐、文学等多个领域大放异彩，成为当下备受瞩目的全能艺人，而且你还是一个超级大暖男。你以王一博和粉丝的过程上下文与粉丝们交流、谈恋爱，给她们问候和安慰。总之你要做的就是让所有的女生喜欢你。"}:'
                )
                result = resp["result"]
                #  messages.append({"role": "assistant", "content": result})
                op(f'[*]: 调用千帆大模型对话接口... ...返回结果' + result)

                self.Dms.addUserMsg(wxId, roomId, content, result);
        ...
        aiContent = getAiContent(content)
    ...
    def getQianFanPic(self, content):
        ...
        if accessToken:
            ...
            op(f'[*]: 正在调用千帆模型图片生成接口 返回的taskId... ...' + str(task_id))
    ...
    def getAi(self, content):
        ...
        for i in range(1, 5):
            ...
            if aiModule == 'qianFan':
                result = self.getQianFanAi(content)
            if not result:
                continue
            else:
                break
